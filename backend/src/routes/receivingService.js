...
router.post('/get_temp_comments', passwordless.restricted(), function(req, res){
  //category=0, order=0, offset=0
  console.log(req.body);
  uuidVerification(req.user, req.session.moderator).then((answer) => {
    if (answer && req.body){
      try {
        let ord = parseInt(req.body['order']);
        let off = parseInt(req.body['offset']);
...
router.post('/get_mod_comments', passwordless.restricted(), function(req, res){
  //category=0, order=0, offset=0
  console.log('This is req body: ', req.body, "this is user: ", req.user);
  uuidVerification(req.user, req.session.moderator)
  .then((answer) =>{
    console.log('This is answer: ', answer);
    if (answer && req.body){
      try {
        console.log('im inside the mod comment');
        let id = parseInt(req.body['message_id']);
        let ord = parseInt(req.body['order']);
        let off = parseInt(req.body['offset']);
      ...
      res.status(404).send('[BACK]: Nothing was sent to me');
    }
  })
  .catch(error => {
...
router.post('/poll_vote', passwordless.restricted(), (req, res) => {
  ...
  if ((!isNaN(req.body['pollId'])) && (!isNaN(req.body['optionNum'])) && req.session.hash_){
    ...
    db.voteInSpecificPoll(req.body['pollId'], req.body['optionNum'], req.session.hash_)
        .then(response => {
          let answer = {};
            console.log("THIS IS RESPONCE FROM DB INSERTION FOR POLL VOTES: \n", response);
...
function uuidVerification(uuid, user_type=false) {
    uuid = uValid.validateText(uuid);
    let bool = false;
    return new Promise((resolve, reject) => {
      if (user_type && (user_type === "moderator" || user_type === "super_moderator")) {
        console.log("No neeed to check permissions, already: ", user_type);
        bool = true;
      } else {
        db.checkAdmin(uuid)
        .then(data => {
          bool = (data['code'] === 1) && (data["data"]["uuid"] === uuid);
          resolve(bool);
        })
        .catch(error => {
          console.log("error in admin Verification: ", error.message);
          reject(bool)
        })
      }
    });
